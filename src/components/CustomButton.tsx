...
interface MyButtonProps {
  ...
  /**
   * HTML button type.
   * Defaults to "button" so the component never accidentally submits a parent
   * <form>. Pass "submit" only when the button IS the form submit action.
   * Without this, putting a CustomButton inside <form onSubmit=...> caused
   * the form to be submitted twice — once by onClick, once by the implicit
   * type="submit" — which is what produced duplicate rows in the contact
   * submissions table.
   */
  type?: "button" | "submit" | "reset";
}

function CustomButton({ variant = "primary", text = "GET STARTED", className = "", onClick, type = "button" }: MyButtonProps) {
