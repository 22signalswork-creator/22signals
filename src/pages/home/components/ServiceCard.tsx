...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <div className={`h-[435px] relative rounded-[24px] overflow-hidden shadow-lg flex flex-col ${customClass}`} >
      ...
      {/* Content Layer */}
      <div className="relative z-20 flex flex-col justify-between h-full p-[30px] md:p-[48px] text-white" style={{background:  "#00000080"}}>
        <div className="space-y-4">
          <img src={icon} alt={title} className="w-[60px] h-[60px]" />
          <h3 className={`text-2xl font-semibold ${titleColorClass}`}>{title}</h3>
          <p className="text-base max-w-[340px] " style={{ color: 'white' }}>{description}</p>
        </div>
