...
// Smoothed cursor — track raw + interpolated positions, paint with translate3d
let cursorRawX = window.innerWidth / 2;
let cursorRawY = window.innerHeight / 2;
let cursorRenderX = cursorRawX;
let cursorRenderY = cursorRawY;
...
document.addEventListener('mousemove', (e) => {
    ...
    cursorRawX = e.clientX; cursorRawY = e.clientY;
}, { passive: true });

function tickCursor() {
    // Lerp toward raw position for buttery follow
    cursorRenderX += (cursorRawX - cursorRenderX) * 0.28;
    cursorRenderY += (cursorRawY - cursorRenderY) * 0.28;
    cursorDot.style.transform = 'translate3d(' + cursorRenderX + 'px, ' + cursorRenderY + 'px, 0) translate(-50%, -50%)';
...
document.addEventListener('touchmove', (e) => {
    // Don't preventDefault — that locks page scrolling on mobile
    const t = e.touches[0];
...
function animate() {
    ...
    const targetRotZ = -mouse.x * 0.10;
    globeGroup.rotation.x += (targetRotX - globeGroup.rotation.x) * 0.07;
    globeGroup.rotation.z += (targetRotZ - globeGroup.rotation.z) * 0.07;
  ...
  return (
    <>
      <style>{`
        ...
        #globe-canvas { position: absolute; top: 0; left: 0; z-index: 0; }
        #spectrum-canvas { position: absolute; top: 0; left: 0; z-index: 1; pointer-events: none; }
        #cursor-dot {
        ...
        /* Hide the cursor-following dot on touch devices — it's a desktop
           hover decoration and looks broken sitting in random spots on mobile. */
      ...
      <div id="ui" ref={uiRef} className="hidden">
        ...
        <div className="ctrl"><label>Emissive Intensity</label><input ref={emissiveIntRef} type="range" id="emissiveInt" min="0.2" max="4.0" step="0.1" defaultValue="3.0" /></div>
        <div className="ctrl"><label>Fresnel Power</label><input ref={fresPowRef} type="range" id="fresPow" min="1.0" max="10.0" step="0.1" defaultValue="3.7" /></div>
        <div className="ctrl"><label>Distort Strength</label><input ref={distortStrRef} type="range" id="distortStr" min="0" max="1.5" step="0.05" defaultValue="1.2" /></div>
        <div className="ctrl"><label>Distort Radius</label><input ref={distortRadRef} type="range" id="distortRad" min="0.1" max="1.5" step="0.05" defaultValue="1.2" /></div>
        <div className="ctrl"><label>Rotation Speed</label><input ref={rotSpeedRef} type="range" id="rotSpeed" min="0" max="0.02" step="0.0005" defaultValue="0.010" /></div>
        <div className="ctrl"><label>Spectrum Height</label><input ref={specHeightRef} type="range" id="specHeight" min="5" max="80" step="1" defaultValue="58" /></div>
        <div className="ctrl"><label>Spectrum Glow</label><input ref={specGlowRef} type="range" id="specGlow" min="0" max="40" step="1" defaultValue="32" /></div>
        <div className="ctrl"><label>Primary Color</label><input ref={primColorRef} type="color" id="primColor" defaultValue="#2266ee" /></div>
        <div className="ctrl"><label>Accent Color</label><input ref={accentColorRef} type="color" id="accentColor" defaultValue="#ffffff" /></div>
